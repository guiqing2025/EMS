  ...
  <div class="erp-content">
    <div class="erp-page-card">
      <div class="erp-page-toolbar">
        <div>
          <h2 style="margin: 0; font-size: 18px">委外入库送检</h2>
          <p style="margin: 4px 0 0; color: var(--erp-text-muted); font-size: 13px">
            判定合格/不合格 → 合格生成委托入库，不合格生成退货
          </p>
        </div>
      ...
      <div class="erp-page-body">
        <el-table v-loading="loading" :data="rows" border stripe size="small">
          ...
          <el-table-column prop="ww_no" label="委外单" width="140" />
    ...
    <el-dialog v-model="judgeOpen" title="委外送检判定" width="640px" destroy-on-close>
...
async function onReceipt(row: InspectDoc) {
  try {
    ...
    ElMessage.success(`已建委托入库 ${r.receipt_no}`)
    await router.push('/outsource/receipt')
  } catch (e) {
...
async function onReturn(row: InspectDoc) {
  try {
    ...
    await router.push('/outsource/return')
  } catch (e) {
    ElMessage.error(e instanceof Error ? e.message : '失败')
  }
}
