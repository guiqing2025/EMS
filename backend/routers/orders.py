...
from schemas import ManualOrderIn, OrderDeleteIn, OrderRemarkUpdate, SrmOrderOut
...
def _serialize_orders(orders: list[SrmOrder], db: Session) -> list[SrmOrderOut]:
    ...
    tooling_map = tooling_summaries_for_orders(db, tooling_pairs)
    ...
    for order, ic in zip(orders, order_ics):
        ...
        payload.pending_ship_qty = stats.get("pending", 0)
        payload.shipped_local_qty = stats.get("shipped", 0)
        ...
        # 列表禁止现场算齐套：只用订单已存 material_status
        ...
        kit_status = compute_customer_kit_status(
            order.collected_sets_qty, order_qty, order.customer_id, material_status
        )
...
def list_orders(
    ...
    keyword: str = "",
    completed: str = Query("all", pattern="^(all|incomplete|completed)$"),
    srm_status: str = "",
    date_from: str = "",
    date_to: str = "",
...
def _filter_params(
    ...
    if incomplete_only:
        completed = "incomplete"
    return keyword, completed, srm_status, date_from, date_to, receive_filter, customer_id, controlled, new_only
...
def export_orders(
    ...
    keyword, completed, srm_status, date_from, date_to, receive_filter, customer_id, controlled, new_only = _filter_params(
        keyword,
        completed,
        srm_status,
        date_from,
