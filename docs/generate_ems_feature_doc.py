...
def build() -> Path:
    ...
    add_table(
        ...
        ["功能", "说明"],
        [
            ["SRM 自动同步", "定时/手动从菲利斯、恩玖、永联等 SRM 拉取在制订单"],
            ["订单查询", "按客户、订单号、料号、品名、日期、SRM 状态、收货状态筛选"],
            ["订单导出", "导出 XLSX 表格"],
            ["手动录单", "无 SRM 的小客户可手工录入，支持后续扫码发货"],
            ["客户齐套", "菲利斯等展示 SRM 齐套信息；结合 EMS BOM 计算备料齐套"],
    ...
    add_table(
        ...
        [
            ...
            ["退料审核", "部门退料申请，仓库确认入库"],
            ["流水账", "来料、发料、退料等全链路记录"],
            ["共享盘导入", "从「菲利斯进销表」等 Excel「库存表」同步库存"],
            ["导出库存", "导出当前库存表"],
    ...
    add_table(
        ...
        ["集成对象", "同步内容", "方式"],
        [
            ["菲利斯 SRM", "采购订单、交货/收货数量", "API 定时同步"],
            ["恩玖 SRM", "订单数据", "API 定时同步"],
            ["永联金蝶 SCP", "订单数据", "API 定时同步"],
    ...
    fr = footer.add_run("深圳鼎雄电子科技有限公司 · EMS 生产管理系统")
    fr.font.size = Pt(9)
